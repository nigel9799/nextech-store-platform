...
- Mandatory TOTP enrollment/challenge. Protected pages and every exposed foundation table require AAL2.
...
These fixtures are deliberately synthetic, are created only through a loopback Supabase URL, and must never be used outside local testing. The setup removes existing fixture MFA factors so the enrollment flow can be repeated.
...
- AAL1 sessions are redirected to enroll or challenge. Restrictive RLS also blocks AAL1 reads and writes if a route guard is bypassed.
...
- A documented identity check for owner MFA recovery. Recovery should require a second authorized owner or a controlled Supabase administrator procedure; no staff member can reset owner MFA through this application.
